    ...
    while True:
        try:    
            ...
            if modificar == 1:
                ...
                producto_modifcar = int(input('Ingresa el codigo del producto a modificar: '))
        
                producto_encontrado = None
                for Producto in productos:
                    if Producto.codigo == producto_modifcar:
                        producto_encontrado = Producto
                        break
                    if producto_encontrado:
                        producto = producto_encontrado
                        cantidad_nueva = int(input(f'Ingrese la nueva cantidad para el producto {producto.nombre}: '))
            
                    if producto.verificar_stock(cantidad_nueva):
                
                        cantidad_actual = carrito.obtener_cantidad_producto(producto)
                        diferencia = cantidad_nueva - cantidad_actual
             
                        producto.modificar_stock(diferencia)
